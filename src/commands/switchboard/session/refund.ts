...
export default class SwitchboardSessionRefund extends Command {
  static description = [
    "Refund an eligible Switchboard developer session.",
    "This native proof entrypoint calls the existing advanced switchboard session refund implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard session refund --session-id 0x...",
    "<%= config.bin %> switchboard session refund --report report.json --developer-private-key-env DEVELOPER_PRIVATE_KEY --yes",
    "<%= config.bin %> switchboard session refund --session-id 0x... --refund-reason unfulfilled --yes --json"
  ];
  static flags = sessionRefundFlags();
  static strict = false;
  static summary = "Refund a Switchboard session.";

  async run(): Promise<void> {
    this.parsed = true;
    if (this.argv.includes("--help") || this.argv.includes("-h")) {
      printSwitchboardSessionRefundHelp(this.config.bin);
      return;
    }
    const exitCode = await runSwitchboardSessionRefundNative(this.argv);
    if (exitCode !== 0) {
      this.exit(exitCode);
    }
  }
...
export async function runSwitchboardSessionRefundNative(
  argv: readonly string[],
  options: SwitchboardSessionRefundOptions = {}
): Promise<number> {
...
function printSwitchboardSessionRefundHelp(bin: string): void {
  console.log(`Refund a Switchboard session.

USAGE
  $ ${bin} switchboard session refund (--session-id <bytes32> | --report <path>) [--yes] [--json]
...
DESCRIPTION
  Advanced alias for the top-level refund command. By default it inspects the
...
EXAMPLES
  $ ${bin} switchboard session refund --session-id 0x...
  $ ${bin} switchboard session refund --report report.json --developer-private-key-env DEVELOPER_PRIVATE_KEY --yes
  $ ${bin} switchboard session refund --session-id 0x... --refund-reason unfulfilled --yes --json`);
}
