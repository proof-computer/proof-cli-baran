...
export default class SwitchboardRefund extends Command {
  static description = [
    ...
    "This native proof entrypoint calls the existing switchboard refund implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard refund --session-id 0x...",
    "<%= config.bin %> switchboard refund --report report.json --developer-private-key-env DEVELOPER_PRIVATE_KEY --yes",
    "<%= config.bin %> switchboard refund --session-id 0x... --refund-reason unfulfilled --yes --json"
  ];
...
export async function runSwitchboardRefundNative(
  ...
  return compatibilityRunner(["refund", ...argv]);
}
...
USAGE
  $ ${bin} switchboard refund (--session-id <bytes32> | --report <path>) [--yes] [--json]
...
EXAMPLES
  $ ${bin} switchboard refund --session-id 0x...
  $ ${bin} switchboard refund --report report.json --developer-private-key-env DEVELOPER_PRIVATE_KEY --yes
  $ ${bin} switchboard refund --session-id 0x... --refund-reason unfulfilled --yes --json`);
}
