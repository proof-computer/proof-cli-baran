...
export default class SwitchboardDeployResume extends Command {
  static description = [
    "Resume a single-replica Switchboard deploy workflow from local private state.",
  ...
  static examples = [
    "<%= config.bin %> switchboard deploy resume --run-dir .switchboard/runs/<id> --yes",
    "<%= config.bin %> switchboard deploy resume --snapshot .switchboard/runs/<id>/switchboard-deploy-workflow.private.json --yes --json",
    "<%= config.bin %> switchboard deploy resume --report report.json --yes --allow-late-funding"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
    ...
    "run-dir": Flags.string({
      description: "Resume from a Switchboard deploy run directory."
    }),
  ...
  static strict = false;
  static summary = "Resume a single-replica Switchboard deploy workflow.";
...
export async function runSwitchboardDeployResumeNative(
  ...
  return runSwitchboardCompatibility(["deploy", "resume", ...argv]);
}
...
async function loadSwitchboardDeployResumeRunner(): Promise<RunSwitchboardDeployResume | undefined> {
  try {
    ...
    return typeof module.runSwitchboardDeployResume === "function"
...
function printSwitchboardDeployResumeHelp(bin: string): void {
  console.log(`Resume a single-replica Switchboard deploy workflow.

USAGE
  $ ${bin} switchboard deploy resume [--report <path> | --run-dir <path> | --snapshot <path>] --yes [--allow-late-funding] [--json]

FLAGS
  --report <path>          Resume from a report.json file with local private state.
  --run-dir <path>         Resume from a Switchboard deploy run directory.
...
EXAMPLES
  $ ${bin} switchboard deploy resume --run-dir .switchboard/runs/<id> --yes
  $ ${bin} switchboard deploy resume --snapshot .switchboard/runs/<id>/switchboard-deploy-workflow.private.json --yes --json
  $ ${bin} switchboard deploy resume --report report.json --yes --allow-late-funding`);
}
