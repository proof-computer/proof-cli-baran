...
export default class SwitchboardPreflight extends Command {
  static description = [
    "Check Switchboard project, manifest, RPC, credential, payment, DNS, and deploy readiness.",
    "This is a read-only inspection command backed by the existing switchboard preflight implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard preflight",
    "<%= config.bin %> switchboard preflight --json",
    "<%= config.bin %> switchboard preflight --project-dir ./app --context mainnet"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
  ...
  static strict = false;
  static summary = "Check Switchboard deploy readiness.";
...
function printSwitchboardPreflightHelp(bin: string): void {
  console.log(`Check Switchboard deploy readiness.

USAGE
  $ ${bin} switchboard preflight [--json]
...
DESCRIPTION
  Read-only readiness inspection for local Switchboard deploys. It may read
...
EXAMPLES
  $ ${bin} switchboard preflight
  $ ${bin} switchboard preflight --json
  $ ${bin} switchboard preflight --project-dir ./app --context mainnet`);
}
