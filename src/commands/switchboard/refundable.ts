...
export default class SwitchboardRefundable extends Command {
  static description = [
    ...
    "This is the native proof entrypoint for the existing read-only switchboard refundable command."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard refundable --session-id 0x...",
    "<%= config.bin %> switchboard refundable --report report.json --json",
    "<%= config.bin %> switchboard refundable --session-id 0x... --refund-reason activation-timeout"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
...
USAGE
  $ ${bin} switchboard refundable --session-id <bytes32> [--json]
  $ ${bin} switchboard refundable --report <path> [--json]
...
EXAMPLES
  $ ${bin} switchboard refundable --session-id 0x...
  $ ${bin} switchboard refundable --report report.json --json
  $ ${bin} switchboard refundable --session-id 0x... --refund-reason activation-timeout`);
}
