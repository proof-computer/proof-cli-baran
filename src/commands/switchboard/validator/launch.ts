...
export default class SwitchboardValidatorLaunch extends Command {
  static description = [
    "Launch Switchboard validators.",
    "This native proof entrypoint calls the existing switchboard validator launch implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard validator launch --processor 5CC2L... --yes",
    "<%= config.bin %> switchboard validator launch --processors 5CC2L...,5Fh... --json",
    "<%= config.bin %> switchboard validator launch --manager-id 9470 --count 2 --dry-run"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
  ...
  static strict = false;
  static summary = "Launch Switchboard validators.";
...
export async function runSwitchboardValidatorLaunchNative(
  ...
  console.error("[switchboard] Error: internal proof switchboard runner runSwitchboardValidatorLaunch is unavailable.");
  return 1;
}
...
function printSwitchboardValidatorLaunchHelp(bin: string): void {
  console.log(`Launch Switchboard validators.

USAGE
  $ ${bin} switchboard validator launch [options]
...
DESCRIPTION
  Launches approved Switchboard validator runtime jobs on selected Acurast
  processors and waits for relay registration unless --dry-run is used.

EXAMPLES
  $ ${bin} switchboard validator launch --processor 5CC2L... --yes
  $ ${bin} switchboard validator launch --processors 5CC2L...,5Fh... --json
  $ ${bin} switchboard validator launch --manager-id 9470 --count 2 --dry-run`);
}
