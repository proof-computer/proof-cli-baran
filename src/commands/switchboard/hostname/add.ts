...
export default class SwitchboardHostnameAdd extends Command {
  static description = [
    "Attach a Switchboard customer hostname.",
    "This native proof entrypoint calls the existing signed switchboard hostname add implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard hostname add app.example.com --report .switchboard/latest-report.json",
    "<%= config.bin %> switchboard hostname add app.example.com --endpoint demo.ingress.example --session-id 0x... --developer-private-key-env DEVELOPER_PRIVATE_KEY",
    "<%= config.bin %> switchboard hostname add --customer-hostname app.example.com --endpoint demo.ingress.example --byo-tls --json"
  ];
...
export async function runSwitchboardHostnameAddNative(
  ...
  return compatibilityRunner(["hostname", "add", ...argv]);
}
...
async function loadSwitchboardHostnameAddRunner(): Promise<RunSwitchboardHostnameAdd | undefined> {
  try {
    ...
    return typeof module.runSwitchboardHostnameAdd === "function"
      ? module.runSwitchboardHostnameAdd
      : undefined;
  } catch {
    return undefined;
  }
...
function printSwitchboardHostnameAddHelp(bin: string): void {
  console.log(`Attach a Switchboard customer hostname.

USAGE
  $ ${bin} switchboard hostname add <hostname> [--report <path>] [--json]
...
EXAMPLES
  $ ${bin} switchboard hostname add app.example.com --report .switchboard/latest-report.json
  $ ${bin} switchboard hostname add app.example.com --endpoint demo.ingress.example --session-id 0x... --developer-private-key-env DEVELOPER_PRIVATE_KEY
  $ ${bin} switchboard hostname add --customer-hostname app.example.com --endpoint demo.ingress.example --byo-tls --json`);
}
