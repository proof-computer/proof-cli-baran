...
export default class SwitchboardHostnameRemove extends Command {
  static description = [
    "Remove a Switchboard customer hostname.",
    "This native proof entrypoint calls the existing signed switchboard hostname remove implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard hostname remove app.example.com --report .switchboard/latest-report.json",
    "<%= config.bin %> switchboard hostname remove app.example.com --endpoint demo.ingress.example --session-id 0x... --developer-private-key-env DEVELOPER_PRIVATE_KEY",
    "<%= config.bin %> switchboard hostname remove --customer-hostname app.example.com --endpoint demo.ingress.example --json"
  ];
...
export async function runSwitchboardHostnameRemoveNative(
  ...
  return compatibilityRunner(["hostname", "remove", ...argv]);
}
...
async function loadSwitchboardHostnameRemoveRunner(): Promise<RunSwitchboardHostnameRemove | undefined> {
  try {
    ...
    return typeof module.runSwitchboardHostnameRemove === "function"
      ? module.runSwitchboardHostnameRemove
      : undefined;
  } catch {
    return undefined;
  }
...
function printSwitchboardHostnameRemoveHelp(bin: string): void {
  console.log(`Remove a Switchboard customer hostname.

USAGE
  $ ${bin} switchboard hostname remove <hostname> [--report <path>] [--json]
...
EXAMPLES
  $ ${bin} switchboard hostname remove app.example.com --report .switchboard/latest-report.json
  $ ${bin} switchboard hostname remove app.example.com --endpoint demo.ingress.example --session-id 0x... --developer-private-key-env DEVELOPER_PRIVATE_KEY
  $ ${bin} switchboard hostname remove --customer-hostname app.example.com --endpoint demo.ingress.example --json`);
}
