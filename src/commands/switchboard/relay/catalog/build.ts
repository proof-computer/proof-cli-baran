...
export default class SwitchboardRelayCatalogBuild extends Command {
  static description = [
    "Build a signed Switchboard relay catalog bundle.",
    "This native proof entrypoint calls the existing local switchboard relay catalog build implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard relay catalog build --output service-catalogs.signed.json",
    "<%= config.bin %> switchboard relay catalog build --specs-dir relays --stdout"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
...
function printSwitchboardRelayCatalogBuildHelp(bin: string): void {
  console.log(`Build a signed Switchboard relay catalog bundle.

USAGE
  $ ${bin} switchboard relay catalog build --output <path>
  $ ${bin} switchboard relay catalog build --specs-dir <dir> --stdout
...
DESCRIPTION
  ...
  transactions, or change local Switchboard context state.

EXAMPLES
  $ ${bin} switchboard relay catalog build --output service-catalogs.signed.json
  $ ${bin} switchboard relay catalog build --specs-dir relays --stdout`);
}
