...
export default class SwitchboardRelayDnsRemove extends Command {
  static description = [
    "Remove Switchboard relay DNS records.",
    "This native proof entrypoint calls the existing switchboard relay dns remove implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard relay dns remove relay-d",
    "<%= config.bin %> switchboard relay dns remove relay-d --spec relays/relay-d.json",
    "<%= config.bin %> switchboard relay dns remove relay-d --yes"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
...
export async function runSwitchboardRelayDnsRemoveNative(
  ...
  console.error("[switchboard] Error: internal proof switchboard runner runSwitchboardRelayDnsRemove is unavailable.");
  return 1;
}
...
function printSwitchboardRelayDnsRemoveHelp(bin: string): void {
  console.log(`Remove Switchboard relay DNS records.

USAGE
  $ ${bin} switchboard relay dns remove <relay-id> [options]
...
EXAMPLES
  $ ${bin} switchboard relay dns remove relay-d
  $ ${bin} switchboard relay dns remove relay-d --spec relays/relay-d.json
  $ ${bin} switchboard relay dns remove relay-d --yes`);
}
