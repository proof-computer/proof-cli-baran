...
export default class SwitchboardRelayPickProcessor extends Command {
  static description = [
    "Inspect Acurast processor availability for a Switchboard relay.",
    "This native proof entrypoint calls the existing switchboard relay pick-processor implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> switchboard relay pick-processor relay-d",
    "<%= config.bin %> switchboard relay pick-processor relay-d --json",
    "<%= config.bin %> switchboard relay pick-processor relay-d --pin auto"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
...
function printSwitchboardRelayPickProcessorHelp(bin: string): void {
  console.log(`Inspect Acurast processor availability for a Switchboard relay.

USAGE
  $ ${bin} switchboard relay pick-processor <relay-id> [options]
...
EXAMPLES
  $ ${bin} switchboard relay pick-processor relay-d
  $ ${bin} switchboard relay pick-processor relay-d --json
  $ ${bin} switchboard relay pick-processor relay-d --pin auto`);
}
