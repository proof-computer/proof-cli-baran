...
export default class SwitchboardCatalogBuild extends Command {
  static description = [
    "Build signed Baran service catalogs.",
    "This is the native proof entrypoint for the existing local baran catalog build command."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran catalog build --spec service-catalogs.json --output service-catalogs.signed.json",
    "<%= config.bin %> baran catalog build --spec service-catalogs.json --stdout",
    "<%= config.bin %> baran catalog build --output service-catalogs.signed.json"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
...
export async function runSwitchboardCatalogBuildNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardCatalogBuild is unavailable.");
  return 1;
}
...
function printSwitchboardCatalogBuildHelp(bin: string): void {
  console.log(`Build signed Baran service catalogs.

USAGE
  $ ${bin} baran catalog build --spec <path> --output <path>
  $ ${bin} baran catalog build --spec <path> --stdout
  $ ${bin} baran catalog build --output <path>
...
DESCRIPTION
  Local service catalog artifact build. It loads the existing Baran
  ...
  relay, submit transactions, deploy jobs, or mutate local Baran context
  state.

EXAMPLES
  $ ${bin} baran catalog build --spec service-catalogs.json --output service-catalogs.signed.json
  $ ${bin} baran catalog build --spec service-catalogs.json --stdout
  $ ${bin} baran catalog build --output service-catalogs.signed.json`);
}
