...
export default class SwitchboardCatalogSetState extends Command {
  static description = [
    "Update local Baran catalog service state.",
    "This is the native proof entrypoint for the existing local baran catalog set-state command."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran catalog set-state relay relay-d active --spec service-catalogs.json --output service-catalogs.signed.json",
    "<%= config.bin %> baran catalog set-state relay-d draining --spec service-catalogs.json --no-rebuild",
    "<%= config.bin %> baran catalog set-state control-api control-bootstrap disabled --spec service-catalogs.json --stdout"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
...
export async function runSwitchboardCatalogSetStateNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardCatalogSetState is unavailable.");
  return 1;
}
...
function printSwitchboardCatalogSetStateHelp(bin: string): void {
  console.log(`Update local Baran catalog service state.

USAGE
  $ ${bin} baran catalog set-state <role> <service-id> <state> --spec <path>
  $ ${bin} baran catalog set-state <service-id> <state> --spec <path> [--role relay]
...
EXAMPLES
  $ ${bin} baran catalog set-state relay relay-d active --spec service-catalogs.json --output service-catalogs.signed.json
  $ ${bin} baran catalog set-state relay-d draining --spec service-catalogs.json --no-rebuild
  $ ${bin} baran catalog set-state control-api control-bootstrap disabled --spec service-catalogs.json --stdout`);
}
