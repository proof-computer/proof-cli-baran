...
export default class SwitchboardCatalogVerify extends Command {
  static description = [
    "Verify signed Baran service catalogs from a network manifest.",
    "This is the native proof entrypoint for the existing read-only baran catalog verify command."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran catalog verify --manifest-url https://control.example/v1/network-manifest --manifest-signer 5...",
    "<%= config.bin %> baran catalog verify --manifest-url https://control.example/v1/network-manifest --manifest-signer 5... --json",
    "<%= config.bin %> baran catalog verify --required relays,control-api"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
...
export async function runSwitchboardCatalogVerifyNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardCatalogVerify is unavailable.");
  return 1;
}
...
function printSwitchboardCatalogVerifyHelp(bin: string): void {
  console.log(`Verify signed Baran service catalogs from a network manifest.

USAGE
  $ ${bin} baran catalog verify --manifest-url <url> --manifest-signer <signer> [--json]
...
DESCRIPTION
  ...
  relay state, or changes local Baran project/context state.

EXAMPLES
  $ ${bin} baran catalog verify --manifest-url https://control.example/v1/network-manifest --manifest-signer 5...
  $ ${bin} baran catalog verify --manifest-url https://control.example/v1/network-manifest --manifest-signer 5... --json
  $ ${bin} baran catalog verify --required relays,control-api`);
}
