...
export default class SwitchboardDeployDoctor extends Command {
  static description = [
    "Diagnose Baran deploy state without spending, deploying, mutating routes, changing DNS, or writing settlement state.",
  ...
  static examples = [
    "<%= config.bin %> baran deploy doctor --run-dir .switchboard/runs/<id>",
    "<%= config.bin %> baran deploy doctor --intent-id di_... --relay-url https://control.switchboard.proof.computer --intent-token-env SWITCHBOARD_INTENT_TOKEN",
    "<%= config.bin %> baran deploy doctor --report report.json --probe --json"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
    ...
    "run-dir": Flags.string({
      description: "Read deploy state from a Baran deploy run directory."
    }),
  ...
  static strict = false;
  static summary = "Diagnose Baran deploy state.";
...
export async function runSwitchboardDeployDoctorNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardDeployDoctor is unavailable.");
  return 1;
}
...
function printSwitchboardDeployDoctorHelp(bin: string): void {
  console.log(`Diagnose Baran deploy state.

USAGE
  $ ${bin} baran deploy doctor [--report <path> | --run-dir <path> | --snapshot <path> | --intent-id <id>] [--probe] [--json]

FLAGS
  --report <path>                     Read deploy state from a report.json file.
  --run-dir <path>                    Read deploy state from a Baran deploy run directory.
...
EXAMPLES
  $ ${bin} baran deploy doctor --run-dir .switchboard/runs/<id>
  $ ${bin} baran deploy doctor --intent-id di_... --relay-url https://control.switchboard.proof.computer --intent-token-env SWITCHBOARD_INTENT_TOKEN
  $ ${bin} baran deploy doctor --report report.json --probe --json`);
}
