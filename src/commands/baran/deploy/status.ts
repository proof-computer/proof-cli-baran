...
export default class SwitchboardDeployStatus extends Command {
  static description = [
    "Read local Baran deploy workflow state without resuming, spending, deploying, or mutating routes.",
  ...
  static examples = [
    "<%= config.bin %> baran deploy status --run-dir .switchboard/runs/<id>",
    "<%= config.bin %> baran deploy status --report report.json --json",
    "<%= config.bin %> baran deploy status --snapshot .switchboard/runs/<id>/switchboard-deploy-workflow.snapshot.json"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
  ...
  static strict = false;
  static summary = "Read local Baran deploy workflow state.";
...
export async function runSwitchboardDeployStatusNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardDeployStatus is unavailable.");
  return 1;
}
...
function printSwitchboardDeployStatusHelp(bin: string): void {
  console.log(`Read local Baran deploy workflow state.

USAGE
  $ ${bin} baran deploy status [--report <path> | --run-dir <path> | --snapshot <path>] [--json]
...
EXAMPLES
  $ ${bin} baran deploy status --run-dir .switchboard/runs/<id>
  $ ${bin} baran deploy status --report report.json --json
  $ ${bin} baran deploy status --snapshot .switchboard/runs/<id>/switchboard-deploy-workflow.snapshot.json`);
}
