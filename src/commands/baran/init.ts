...
export default class SwitchboardInit extends Command {
  static description = [
    ...
    "This native proof entrypoint calls the existing baran init implementation, including the SSH template scaffold."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran init --project hello-api --context mainnet",
    "<%= config.bin %> baran init --template ssh --distro ubuntu",
    "<%= config.bin %> baran init --project-dir ./app --force --json"
  ];
...
export async function runSwitchboardInitNative(
  ...
  if (removedEndpointError) {
    console.error(`[baran] ${removedEndpointError}`);
    return 1;
  }
...
function removedInitEndpointArgsMessage(argv: readonly string[]): string | undefined {
  ...
  return (
    ...
    "Canonical PROOF endpoints are allocated during deploy; attach customer domains after deploy with `proof baran hostname add`."
  );
}
...
export function initFlags() {
  return {
    ...
    project: Flags.string({ description: "Baran project name." }),
    name: Flags.string({ description: "Alias for --project." }),
    context: Flags.string({ description: "Baran context name to store in switchboard.json." }),
...
export function printSwitchboardInitHelp(bin: string, command: "init" | "project init"): void {
  console.log(`Initialize a local Baran project.

USAGE
  $ ${bin} baran ${command} [options]

FLAGS
  --project <name>                Baran project name.
  --name <name>                   Alias for --project.
  --context <name>                Baran context name.
...
EXAMPLES
  $ ${bin} baran ${command} --project hello-api --context mainnet
  $ ${bin} baran ${command} --template ssh --distro ubuntu
  $ ${bin} baran ${command} --project-dir ./app --force --json`);
}
