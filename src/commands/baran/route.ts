...
export default class SwitchboardRoute extends Command {
  ...
  static examples = [
    "<%= config.bin %> baran route --target proof-ingress-local --route-id 0x... --json",
    "<%= config.bin %> baran route --target proof-ingress-local --hostname app.example.com --route-class-id 42 --owner 5... --json"
  ];
  static strict = false;
  static flags = {
