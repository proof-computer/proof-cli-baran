...
export default class SwitchboardDeploy extends Command {
  static description = [
    "Deploy a project workload through Baran.",
    "This is the native proof entrypoint for the existing baran deploy workflow. It can spend ACU and the configured Hub payment asset, so --yes is required unless --dry-run is used."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran deploy --entrypoint src/index.ts --yes",
    "<%= config.bin %> baran deploy --runtime script --ssh-public-key-file ~/.ssh/id_ed25519.pub --yes",
    "<%= config.bin %> baran deploy --dry-run --entrypoint src/index.ts --json"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
  ...
  static strict = false;
  static summary = "Deploy a project workload through Baran.";
...
export async function runSwitchboardDeployNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardDeploy is unavailable.");
  return 1;
}
...
function printSwitchboardDeployHelp(bin: string): void {
  console.log(`Deploy a project workload through Baran.

USAGE
  $ ${bin} baran deploy --entrypoint <path> --yes [--json]
  $ ${bin} baran deploy --dry-run --entrypoint <path> [--json]
...
DESCRIPTION
  Native proof CLI entrypoint for the existing baran deploy workflow.
...
EXAMPLES
  $ ${bin} baran deploy --entrypoint src/index.ts --yes
  $ ${bin} baran deploy --runtime script --ssh-public-key-file ~/.ssh/id_ed25519.pub --yes
  $ ${bin} baran deploy --dry-run --entrypoint src/index.ts --json`);
}
