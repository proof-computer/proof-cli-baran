...
export default class SwitchboardGatewayDiscover extends Command {
  static description = [
    ...
    "This native proof entrypoint calls the existing baran gateway discover implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran gateway discover --manager-id 9470 --public-address 203.0.113.10",
    "<%= config.bin %> baran gateway discover --manager-id 9470 --limit 3 --json"
  ];
  static flags = {
...
export async function runSwitchboardGatewayDiscoverNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardGatewayDiscover is unavailable.");
  return 1;
}
...
USAGE
  $ ${bin} baran gateway discover --manager-id <id> --public-address <ip-or-host> [options]
...
EXAMPLES
  $ ${bin} baran gateway discover --manager-id 9470 --public-address 203.0.113.10
  $ ${bin} baran gateway discover --manager-id 9470 --limit 3 --json`);
}
