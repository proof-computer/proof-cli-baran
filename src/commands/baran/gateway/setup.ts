...
export default class SwitchboardGatewaySetup extends Command {
  static description = [
    "Prepare a Baran gateway host.",
    "This native proof entrypoint calls the existing baran gateway setup implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran gateway setup",
    "<%= config.bin %> baran gateway setup --manager-address 5... --manager-id 9470 --generate-report-seed --prepare-admission",
    "<%= config.bin %> baran gateway setup --admission-file operator-admission.json --yes"
  ];
  static flags = {
  ...
  static strict = false;
  static summary = "Prepare a Baran gateway host.";
...
export async function runSwitchboardGatewaySetupNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardGatewaySetup is unavailable.");
  return 1;
}
...
function printSwitchboardGatewaySetupHelp(bin: string): void {
  console.log(`Prepare a Baran gateway host.

USAGE
  $ ${bin} baran gateway setup [options]
...
EXAMPLES
  $ ${bin} baran gateway setup
  $ ${bin} baran gateway setup --manager-address 5... --manager-id 9470 --generate-report-seed --prepare-admission
  $ ${bin} baran gateway setup --admission-file operator-admission.json --yes`);
}
