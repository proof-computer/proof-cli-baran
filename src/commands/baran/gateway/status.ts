import { runSwitchboardGatewayStatus as defaultRunSwitchboardGatewayStatusRunner } from "../../../switchboard-core/cli/src/index.js";
...
export default class SwitchboardGatewayStatus extends Command {
  static description = [
    "Show local Baran gateway stack status.",
    "This native proof entrypoint calls the existing baran gateway status implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran gateway status",
    "<%= config.bin %> baran gateway status --project-dir /srv/proof --json"
  ];
  static flags = {
...
export async function runSwitchboardGatewayStatusNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardGatewayStatus is unavailable.");
  return 1;
}

async function loadSwitchboardGatewayStatusRunner(): Promise<RunSwitchboardGatewayStatus | undefined> {
  return defaultRunSwitchboardGatewayStatusRunner;
}
...
function printSwitchboardGatewayStatusHelp(bin: string): void {
  console.log(`Show local Baran gateway stack status.

USAGE
  $ ${bin} baran gateway status [options]
...
EXAMPLES
  $ ${bin} baran gateway status
  $ ${bin} baran gateway status --project-dir /srv/proof --json`);
}
