...
export default class SwitchboardRelayLogs extends Command {
  static description = [
    "Read encrypted Baran relay log events.",
    "This native proof entrypoint calls the existing baran relay logs implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran relay logs relay-d",
    "<%= config.bin %> baran relay logs relay-d --json",
    "<%= config.bin %> baran relay logs --read-url https://relay.example/v1/log-sinks/sink/events --limit 20"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
...
export async function runSwitchboardRelayLogsNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardRelayLogs is unavailable.");
  return 1;
}
...
function printSwitchboardRelayLogsHelp(bin: string): void {
  console.log(`Read encrypted Baran relay log events.

USAGE
  $ ${bin} baran relay logs [relay-id] [options]
...
EXAMPLES
  $ ${bin} baran relay logs relay-d
  $ ${bin} baran relay logs relay-d --json
  $ ${bin} baran relay logs --read-url https://relay.example/v1/log-sinks/sink/events --limit 20`);
}
