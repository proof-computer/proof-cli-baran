...
export default class SwitchboardRelayBackfillSpecs extends Command {
  static description = [
    "Backfill local Baran relay specs from discovery.",
    "This native proof entrypoint calls the existing local baran relay backfill-specs implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran relay backfill-specs --target bootstrap",
    "<%= config.bin %> baran relay backfill-specs --target acurast --dry-run",
    "<%= config.bin %> baran relay backfill-specs --manifest-url https://control.example/v1/network-manifest"
  ];
  static flags = {
    help: Flags.help({
      char: "h"
...
export async function runSwitchboardRelayBackfillSpecsNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardRelayBackfillSpecs is unavailable.");
  return 1;
}
...
function printSwitchboardRelayBackfillSpecsHelp(bin: string): void {
  console.log(`Backfill local Baran relay specs from discovery.

USAGE
  $ ${bin} baran relay backfill-specs [options]
...
EXAMPLES
  $ ${bin} baran relay backfill-specs --target bootstrap
  $ ${bin} baran relay backfill-specs --target acurast --dry-run
  $ ${bin} baran relay backfill-specs --manifest-url https://control.example/v1/network-manifest`);
}
