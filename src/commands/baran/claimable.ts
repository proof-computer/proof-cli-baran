...
export default class SwitchboardClaimable extends Command {
  static description = [
    "Check released Baran rewards without submitting a claim transaction.",
    "This is the native proof entrypoint for the existing read-only baran claimable command."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran claimable --recipient 0x...",
    "<%= config.bin %> baran claimable --recipient 0x... --json",
    "<%= config.bin %> baran claimable --recipient 0x... --asset 0x..."
  ];
  static flags = {
    help: Flags.help({
      char: "h"
  ...
  static strict = false;
  static summary = "Check Baran claimable rewards.";
...
export async function runSwitchboardClaimableNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardClaimable is unavailable.");
  return 1;
}
...
function printSwitchboardClaimableHelp(bin: string): void {
  console.log(`Check Baran claimable rewards.

USAGE
  $ ${bin} baran claimable --recipient <address> [--json]
...
EXAMPLES
  $ ${bin} baran claimable --recipient 0x...
  $ ${bin} baran claimable --recipient 0x... --json
  $ ${bin} baran claimable --recipient 0x... --asset 0x...`);
}
