...
export default class SwitchboardSessionRegister extends Command {
  static description = [
    "Register a funded Baran session through a relay.",
    "This native proof entrypoint calls the existing baran session register implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran session register --session-id 0x... --relay-url https://control.switchboard.proof.computer --job-signer-private-key 0x... --yes",
    "<%= config.bin %> baran session register --session-id 0x... --relay-url https://control.switchboard.proof.computer --job-signer-private-key 0x... --yes --json"
  ];
  static flags = sessionRegisterFlags();
  static strict = false;
  static summary = "Register a funded Baran session.";
...
export async function runSwitchboardSessionRegisterNative(
  ...
  console.error("[baran] Error: internal proof baran runner runSwitchboardSessionRegister is unavailable.");
  return 1;
}
...
function printSwitchboardSessionRegisterHelp(bin: string): void {
  console.log(`Register a funded Baran session through a relay.

USAGE
  $ ${bin} baran session register --session-id <bytes32> --relay-url <url> --job-signer-private-key <key> --yes [--json]
...
EXAMPLES
  $ ${bin} baran session register --session-id 0x... --relay-url https://control.switchboard.proof.computer --job-signer-private-key 0x... --yes
  $ ${bin} baran session register --session-id 0x... --relay-url https://control.switchboard.proof.computer --job-signer-private-key 0x... --yes --json`);
}
