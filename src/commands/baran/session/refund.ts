...
export default class SwitchboardSessionRefund extends Command {
  static description = [
    ...
    "This native proof entrypoint calls the existing advanced baran session refund implementation."
  ].join("\n");
  static examples = [
    "<%= config.bin %> baran session refund --session-id 0x...",
    "<%= config.bin %> baran session refund --report report.json --developer-private-key-env DEVELOPER_PRIVATE_KEY --yes",
    "<%= config.bin %> baran session refund --session-id 0x... --refund-reason unfulfilled --yes --json"
  ];
...
USAGE
  $ ${bin} baran session refund (--session-id <bytes32> | --report <path>) [--yes] [--json]
...
EXAMPLES
  $ ${bin} baran session refund --session-id 0x...
  $ ${bin} baran session refund --report report.json --developer-private-key-env DEVELOPER_PRIVATE_KEY --yes
  $ ${bin} baran session refund --session-id 0x... --refund-reason unfulfilled --yes --json`);
}
